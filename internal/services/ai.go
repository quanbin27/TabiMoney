...
const (
	defaultAIServiceTimeout  = 60 * time.Second
	categorySuggestTimeout   = 8 * time.Second
	maxErrorBodyPreviewBytes = 2048
)

func (s *AIService) postJSON(ctx context.Context, path string, payload interface{}, out interface{}) error {
	if ctx == nil {
	...
	if err != nil {
		return fmt.Errorf("failed to create AI service request: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		snippet, _ := io.ReadAll(io.LimitReader(resp.Body, maxErrorBodyPreviewBytes))
		if len(snippet) > 0 {
			return fmt.Errorf("AI service %s returned status %d: %s", path, resp.StatusCode, strings.TrimSpace(string(snippet)))
		}
	...
	if err := json.NewDecoder(resp.Body).Decode(out); err != nil {
		return fmt.Errorf("failed to decode AI service %s response: %w", path, err)
	}

	return nil
}

func (s *AIService) tryGetCache(ctx context.Context, key string, dst interface{}) bool {
	if ctx == nil {
		ctx = context.Background()
	}

	cached, err := database.GetCache(ctx, key)
	if err != nil || cached == "" {
		return false
	}
	...
	if err := json.Unmarshal([]byte(cached), dst); err != nil {
		log.Printf("AIService cache decode failed for key %s: %v", key, err)
		return false
	}
...
func (s *AIService) cacheJSON(ctx context.Context, key string, value interface{}, ttl time.Duration) []byte {
	if ctx == nil {
		ctx = context.Background()
	}
	...
	if err != nil {
		log.Printf("AIService cache marshal failed for key %s: %v", key, err)
		return nil
	}

	if err := database.SetCache(ctx, key, data, ttl); err != nil {
		log.Printf("AIService cache write failed for key %s: %v", key, err)
	}
...
func NewAIService(cfg *config.Config) *AIService {
	...
	svc := &AIService{
		config:       cfg,
		db:           database.GetDB(),
		httpClient:   &http.Client{Timeout: defaultAIServiceTimeout},
...
func (s *AIService) ProcessNLU(req *models.NLURequest) (*models.NLUResponse, error) {
	...
	var cachedResp models.NLUResponse
	if s.tryGetCache(ctx, cacheKey, &cachedResp) {
	...
	if err := s.postJSON(ctx, "/nlu/process", payload, &aiResponse); err != nil {
		return nil, err
	}

	resp := &models.NLUResponse{
	...
	// Cache response
	s.cacheJSON(ctx, cacheKey, resp, time.Hour)
...
func (s *AIService) PredictExpenses(req *models.ExpensePredictionRequest) (*models.ExpensePredictionResponse, error) {
	...
	if s.tryGetCache(ctx, cacheKey, &response) {
		return &response, nil
	}
	...
	payload := map[string]interface{}{
		...
		"end_date":   req.EndDate.Format(time.RFC3339),
	}

	if err := s.postJSON(ctx, "/prediction/expenses", payload, &response); err != nil {
		log.Printf("AI Service expense prediction failed: %v", err)
		return nil, err
	}

	// Cache response and persist analysis
	data := s.cacheJSON(ctx, cacheKey, response, 6*time.Hour)
	if data == nil {
	...
	analysis := &models.AIAnalysis{
		...
		Data:            stringMust(data),
		ConfidenceScore: response.ConfidenceScore,
		ModelVersion:    "ai-service",
	}
	if err := s.db.Create(analysis).Error; err != nil {
		log.Printf("Failed to persist expense prediction analysis: %v", err)
	}
...
func (s *AIService) DetectAnomalies(req *models.AnomalyDetectionRequest) (*models.AnomalyDetectionResponse, error) {
	...
	if err := s.postJSON(ctx, "/anomaly/detect", payload, &aiResp); err != nil {
		return nil, err
	}
...
func (s *AIService) SuggestCategory(req *models.CategorySuggestionRequest) (*models.CategorySuggestionResponse, error) {
	...
	ctx, cancel := context.WithTimeout(baseCtx, categorySuggestTimeout)
	defer cancel()

	var response models.CategorySuggestionResponse
	if err := s.postJSON(ctx, "/categorization/suggest", payload, &response); err != nil {
		...
		if len(fallback.Suggestions) == 0 && len(categories) > 0 {
			...
			bestCount := -1
			for _, c := range categories {
				if cnt := freq[c.ID]; cnt > bestCount {
					best = c
					bestCount = cnt
				}
			}
...
func (s *AIService) fetchDynamicSpendingInsights(userID uint64, patterns []models.SpendingPattern) ([]string, []string) {
	...
	if err := s.postJSON(ctx, "/analysis/spending", payload, &out); err != nil {
...
func (s *AIService) ProcessChat(req *models.ChatRequest) (*models.ChatResponse, error) {
	...
	if err := s.postJSON(context.Background(), "/chat/process", payload, &response); err != nil {
		return nil, err
	}
...
func (s *AIService) generateSpendingRecommendations(patterns []models.SpendingPattern) []string {
	...
	for i := 0; i < topN; i++ {
		share := 0.0
		if total > 0 {
			share = (sorted[i].TotalAmount / total) * 100
		}
		if share >= 20 {
